...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("soc", 1, 10062.111801242236, id="soc, one phase"),
        pytest.param("amount", 2, 447.2049689440994, id="amount, two phases"),
    ])
def test_calculate_duration(selected: str, phases: int, expected_duration: float):
    ...
    # execution
    duration = ct.calculate_duration(plan, 60, 45000, 200, phases)
...
def test_scheduled_charging_calc_current(plan_data: SelectedPlan,
    ...
    # execution
    ret = ct.scheduled_charging_calc_current(plan_data, 3600, soc, used_amount, 3, 3, 6)
